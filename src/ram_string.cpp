...
std::unique_ptr<char[]> RAMString::read( std::uint32_t address, std::uint32_t count ) const noexcept
{
  ...
  _CopyString:
    if ( char_read ) {
      std::unique_ptr<char[]> tmp( new char[char_read + length + 1] );

      if ( str_buf )
        std::memcpy( tmp.get(), str_buf.get(), length );

      std::memcpy( tmp.get() + length, (char *)( block->data.get() + begin ) + address & 0b11, char_read );

      length += char_read;

      str_buf = std::move( tmp );
    } // if ( char_read )
...
void RAMString::write( std::uint32_t address, char const *src, std::uint32_t count ) noexcept
{
  ...
  while ( !eof || count ) {
    if ( in_memory ) { // [1]
      ...
      std::FILE *block = std::fopen( file_name, "r+b" );

      assert( block && "Coudln't open file!" );

      auto const begin = address - block.base_address;
      auto const limit = ( RAM::block_size - begin ) * sizeof( std::uint32_t );
      auto const size  = count < limit ? count : limit;

      std::fseek( block, sizeof( std::uint32_t ) * begin + char_written + address & 0b11, SEEK_SET );

      std::fwrite( src, sizeof( char ), size, block );
